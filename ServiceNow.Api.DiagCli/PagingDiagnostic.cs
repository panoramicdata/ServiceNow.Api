...
internal sealed class PagingDiagnostic(IOptions<Configuration> options, ILogger<PagingDiagnostic> logger) : IDiagnostic
{
	...
	public async Task ExecuteAsync(DiagnosticTest test)
	{
		...
		logger.LogInformation($"Starting {nameof(PagingDiagnostic)}");
		...
		try
		{
			using var client = new ServiceNowClient(
				...
				new Options { ValidateCountItemsReturned = true, ValidateCountItemsReturnedTolerance = 0, PageSize = test.PageSize, Logger = logger });

			var results = await client.GetAllByQueryAsync(test.Table, test.Query, fieldList: test.Fields).ConfigureAwait(false);
			logger.LogInformation("Got {ResultCount} results", results.Count);
			...
			logger.LogInformation("Found {DupesCount} dupes - total retrieved = {ResultsCount} - unique = {UniqueCount}",
